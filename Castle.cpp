...
void Castle::Act_Castle_Fighters(Priority_queue<Fighter*>& Q_fighters, int N, Queue<Enemy*>& L_Killed, Stack<Healer*>& S_Healers, Queue<Freezer*>& Q_freezers, int TimeStep, bool THROWICE)//return false if no one to kill
{
	if (Q_fighters.getSize() >= N)
	{
		...
		for (int i = 0; i < N; i++)
		{
			...
			if (fighter->getFirstShotDelay() == -1)
			{
		...
		for (int i = 0; i < temp.getSize(); i++)
		{
			...
			double priority = 0.5 * health_ * power_ * distance_;
	...
	else if ((Q_fighters.getSize() + S_Healers.getSize()) < N)
	{
		...
		if (N - (Q_fighters.getSize() + S_Healers.getSize()) < Q_freezers.getSize())
		{
			...
			for (int i = 0; i < N - (Q_fighters.getSize() + S_Healers.getSize()); i++)
			{
				...
				if (freezer->getFirstShotDelay() == -1)
				{
